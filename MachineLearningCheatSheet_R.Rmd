...
|----------|-------------|---------|
| `cor()` | Calculate correlation coefficient. | `cor(x, y)` |
| `lm()` | Train a linear regression model. | `lm(sheight ~ fheight, data = father.son)` |
| `summary()` | Summarize a linear model. | `m <- lm(y ~ x)`\break`summary(m)` |
| `confint()` | Compute confidence interval for model parameters. | `m <- lm(y ~ x)`\break`confint(m)` |
